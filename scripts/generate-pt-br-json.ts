...
import {
  allowedLinkTypes,
  fetchRoadmapJson,
} from './editor-roadmap-content-json';
...
for (const roadmapId of editorRoadmapIds) {
  ...
  const contentStats = await fs.stat(roadmapContentDir).catch(() => null);
  if (!contentStats || !contentStats.isDirectory()) {
    continue;
  }
  ...
  for (const node of nodes) {
    ...
    const h1Element = rootHtml.querySelector('h1');
    const extractedTitle =
      h1Element?.textContent?.trim() || (node.data.label as string);
  ...
  await fs.writeFile(
    path.join(publicRoadmapsContentDir, `${roadmapId}-pt-br.json`),
