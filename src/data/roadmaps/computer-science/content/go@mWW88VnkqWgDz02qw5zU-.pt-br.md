# Ir

Go é uma linguagem de programação de código aberto suportada pelo Google. Go pode ser usado para escrever serviços em nuvem, ferramentas CLI, usadas para desenvolvimento de API e muito mais.

Visite os seguintes recursos para saber mais:

- [@roadmap@Visit Dedicated Go Roadmap](https://roadmap.sh/golang)
- [@official@A Tour of Go – Noções básicas](https://go.dev/tour/welcome/1)
- [@official@Go Documentação de referência](https://go.dev/doc/)
- [@article@Go by Example - exemplos de programas anotados](https://gobyexample.com/)
- [@article@W3Schools Go Tutorial](https://www.w3schools.com/go/)
- [@article@Criando uma API RESTful JSON em Go](https://thenewstack.io/make-a-restful-json-api-go/)
- [@article@Go, a linguagem de programação da nuvem](https://thenewstack.io/go-the-programming-language-of-the-cloud/)
- [@feed@Explore as principais postagens sobre Golang](https://app.daily.dev/tags/golang?ref=roadmapsh)