# Bancos de dados SQL

SQL significa Linguagem de Consulta Estruturada. É usado para bancos de dados relacionais. Um banco de dados SQL é uma coleção de tabelas que armazena um conjunto específico de dados estruturados. Exemplos de bancos de dados SQL incluem MariaDB, MySQL e PostgreSQL.

Visite os seguintes recursos para saber mais:

- [@roadmap@Visit Roteiro SQL Dedicado](https://roadmap.sh/sql)
- [@artigo@O que é SQL? -AWS](https://aws.amazon.com/what-is/sql/)
- [@article@Bancos de dados SQL](https://www.openlogic.com/blog/what-sql-database)
- [@feed@Explore as principais postagens sobre SQL](https://app.daily.dev/tags/sql?ref=roadmapsh)